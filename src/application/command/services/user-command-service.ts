...
export const createUserCommandService = (
    ...
    try {
      ...
      if (isTechnicalException(err)) {
        if (err.type === TechnicalExceptionType.UNIQUE_VIOLATION_EMAIL) {
          throw BusinessException(BusinessExceptionType.EMAIL_ALREADY_IN_USE);
        }
        if (err.type === TechnicalExceptionType.UNIQUE_VIOLATION_USERNAME) {
          throw BusinessException(BusinessExceptionType.USERNAME_ALREADY_IN_USE);
        }
        if (err.type === TechnicalExceptionType.UNIQUE_VIOLATION_CONTACT) {
          throw BusinessException(BusinessExceptionType.CONTACT_ALREADY_IN_USE);
        }
      }
      throw err;
    }
  ...
  const updateAdmin = async (dto: UpdateAdminDto): Promise<AdminDto> => {
    ...
    if (!foundUser) {
      throw BusinessException(BusinessExceptionType.USER_NOT_FOUND);
    }
    ...
    // 4. 아파트 정보 수정
    if (!foundApartment) {
      throw BusinessException(BusinessExceptionType.APARTMENT_NOT_FOUND);
    }
